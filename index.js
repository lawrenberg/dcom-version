...
var program = commander
    ...
    .option('-i, --milestones', 'List open milestones')
...
Promise.resolve(config)
    ...
    .then(function () {
        ...
        return promptUserFor(basicprompt, program)
    ...
    return gitExec({
        ...
        .then(function (modifiedFiles) {
            if (modifiedFiles.length) {
                ...
                console.log('');
                modifiedFiles.map(function (file) {
                    console.log('\t', file);
                });
                process.exit();
            }
...
function mergeBranches(options) {
    ...
    var branches = options.branches || [].concat(
            prepBranches,
            config.pullRequests.map(function (b) {
                return b.head.ref;
            })
        );
    ...
    return gitExec({
        ...
        .then(function (versions) {
            return versions.filter(function (v) {
                return v.indexOf(program.autoVersion + '.') === 0;
            });
        })
        ...
        .then(function (patches) {
            patches = patches.sort(function (b, a) {
...
function listMilestones() {
    if (program.milestones) {
        ...
        return new Promise(function (resolve, reject) {
             ...
             * Get all milestones
             */
            config.github.issues.getAllMilestones({
                user: config.repository.owner,
                repo: config.repository.name,
                state: 'closed',
                sort: 'due_date'
                ...
                if (err) {
                    console.error("Unable to retrieve milestone list", err)
                }

                resolveDisplay(res.data)
            });
...
function promptUserFor(properties, values) {
    ...
    var keysToPrompt = keys.filter(function (k) {
